...
type PostRostersIDRearrangeURL struct {
	ID string
...
func (o *PostRostersIDRearrangeURL) Build() (*url.URL, error) {
	...
	var _path = "/rosters/{id}/rearrange"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on PostRostersIDRearrangeURL")
	}
...
func (o *PostRostersIDRearrangeURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PostRostersIDRearrangeURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PostRostersIDRearrangeURL")
	}
