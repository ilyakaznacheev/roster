...
type PostRostersIDAddPlayerURL struct {
	ID int64
...
func (o *PostRostersIDAddPlayerURL) Build() (*url.URL, error) {
	...
	var _path = "/rosters/{id}/add_player"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on PostRostersIDAddPlayerURL")
	}
...
func (o *PostRostersIDAddPlayerURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PostRostersIDAddPlayerURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PostRostersIDAddPlayerURL")
	}
