...
func (o *GetRostersIDURL) Build() (*url.URL, error) {
	...
	var _path = "/rosters/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetRostersIDURL")
	}
...
func (o *GetRostersIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetRostersIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetRostersIDURL")
	}
