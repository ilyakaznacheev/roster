...
func (o *GetRostersIDActiveURL) Build() (*url.URL, error) {
	...
	var _path = "/rosters/{id}/active"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetRostersIDActiveURL")
	}
...
func (o *GetRostersIDActiveURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetRostersIDActiveURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetRostersIDActiveURL")
	}
